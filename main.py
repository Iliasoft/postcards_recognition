...
from postcards.GoogleLandmarksDS import GoogleLandmarksDS
from postcards.img_comparators import (
    OpenAIImageComparator,
    OpenCVHistogramBasedImageComparator,
    OpenCVTemplateBasedImageComparator,
    SIFTImageComparator,
)
...
if __name__ == "__main__":
    ...
    possible_image_pairs = BinaryComparisionMatrix.get_pairs(
        np.arange(len(images_to_compare))
    )

    print(
        f"All images:{len(myDS.images)}, Images with landmarks:{len(myDS.images2landmarks)}, "
        f"Unique images with landmarks: {len(images_to_compare)}, "
        f"Landmarks:{len(myDS.landmarks2images)}, Possible pairs:{len(possible_image_pairs)}"
    )

    for comparator in images_comparators:
        similarity_predictions = comparator.get_similarity_score(
            images_to_compare, myDS.get_image_full_path
        )
        possible_image_pairs = BinaryComparisionMatrix.get_pairs(
            np.arange(similarity_predictions.shape[0])
        )
        ...
        for possible_image_pair in possible_image_pairs:
            ...
            prediction = (
                similarity_predictions[possible_image_pair[0], possible_image_pair[1]]
                >= comparator.get_threshold()
            )
        ...
        print(
            f"Prediction results for {comparator.get_name()}, Precision: {tp / (tp + fp):.3f}, "
            f"Recall: {tp / (tp + fn):.3f}, True links: {len(actually_linked_images)}, "
            f"[TP:{tp}, FN:{fn}, FP:{fp}, TN:{tn}], TP-FN-FP = {tp - fn- fp}"
        )
